...
class Pad:
    def __call__(self, image):
        ...
        w, h = 28, 28
        max_wh = (
            # hard-coded to the downloaded data in tMNIST instead of np.max([w, h])
            40
        )
        hp = int((max_wh - w) / 2)
        vp = int((max_wh - h) / 2)
...
class GoldiproxDatamodule(pl.LightningDataModule):
    def __init__(
        ...
        valset_fraction=1.0,
        percent_clean=None,
        cluster_sub_labels=False,
        num_sub_labels=10,  # Number of sub-labels per label
        ...
        super().__init__()
        self.cluster_sub_labels = cluster_sub_labels
        self.num_sub_labels = num_sub_labels
        self.cluster_assignments = None  # Store clustering results
    ...
    def setup(self, stage=None, double_irlomo=False):
        ...
        if double_irlomo:
            ...
            # CIFAR-10 and CIFAR-100 are subsetted before, and Subset objects don't have .targets attribute
            ...
            for t in unique_targets:
                ...
                train_split_1.extend(
                    target_indices[: int(len(target_indices) / 2)])
                train_split_2.extend(
                    target_indices[int(len(target_indices) / 2):])

            self.indices_train_split_1 = Subset(
                indices_train_copy, train_split_1)
            self.indices_train_split_2 = Subset(
                indices_train_copy, train_split_2)
    ...
    def add_embedding(self, embedding_generator):
        self.embedding_generator = embedding_generator
        if self.cluster_sub_labels and self.embedding_generator is not None:
            self.cluster_assignments = self._cluster_labels(self.indices_train)
            self.sub_label_mapping = self.create_sub_label_mapping()

    def _cluster_labels(self, dataset):
        ...
        for label in np.unique(targets):
            ...
            label_data_tensor = torch.from_numpy(
                label_data).float().permute(0, 3, 1, 2)

            # Process embeddings in mini-batches to avoid memory overflow
            batch_size = 1000
            embeddings_list = []

            with torch.no_grad():  # Disable gradient tracking to save memory
                for i in range(0, len(label_data_tensor), batch_size):
                    batch = label_data_tensor[i: i +
                                              batch_size].to("cuda", non_blocking=True)
                    embeddings = self.embedding_generator.get_embedding(batch)
                    # Move to CPU immediately
                    embeddings_list.append(embeddings.cpu())

            embeddings = torch.cat(
                embeddings_list, dim=0).numpy()  # Convert to NumPy

            # Apply Clustering
            clustering = SpectralClustering(
                n_clusters=self.num_sub_labels,  # Number of clusters
                affinity='nearest_neighbors',  # Graph-based affinity
                random_state=42
            )
            sub_labels = clustering.fit_predict(embeddings)

            # Assign new sub-labels
            for idx, sub_label in zip(label_indices, sub_labels):
                cluster_assignments[idx] = int(sub_label * 10 + label)

            print(f"Finished clustering label {label}")

        return cluster_assignments

    def create_sub_label_mapping(self):
        if not self.cluster_assignments:
            raise ValueError(
                "Cluster assignments are not initialized. Ensure `setup` is called with clustering enabled.")

        sub_label_mapping = {}
        for idx, sub_label in self.cluster_assignments.items():
            sub_label_mapping[idx] = sub_label

        return sub_label_mapping

    def _dl(self, ds, bs, shuffle):
        return DataLoader(
            ds,
            batch_size=bs,
            shuffle=shuffle,
    ...
    def train_dataloader(self):
        return self._dl(self.indices_train, self.batch_size, self.shuffle)

    def val_dataloader(self):
        return self._dl(self.indices_val, self.test_batch_size, False)

    def test_dataloader(self):
        return self._dl(self.indices_test, self.test_batch_size, False)
    ...
    def percentage_corrupted(self, global_index, set="train"):
        ...
        return np.in1d(
            global_index.cpu().numpy(
            ), corruption_info["corrupted_points_ndarray"]
        ).mean()
...
class CIFAR10DataModule(GoldiproxDatamodule):
    def __init__(self, *args, **kwargs):
        ...
        train_subset = [x for i in range(split - 1)
                        for x in range(i, trainplusvalset_size, split)]
        val_subset = list(range(split - 1, trainplusvalset_size, split))
        val_subset = [x for i in range(self.number_of_models)
                      for x in val_subset[i::self.number_of_models]]
        ...
        if self.valset_fraction < 1:
            ...
            for t in unique_targets:
                ...
                reduced_val_subset.extend(
                    target_indices_in_valset[: int(
                        len(target_indices_in_valset) * self.valset_fraction)]
                )
        ...
        # ensure that the train and validation subset are disjoint
        assert len(set(val_subset) & set(train_subset)) == 0
    ...
    def __init__(self, *args, **kwargs):
        ...
        if self.valset_fraction < 1:
            ...
            for t in unique_targets:
                ...
                val_subset.extend(
                    target_indices[: int(
                        len(target_indices) * self.valset_fraction)]
                )
...
class Clothing1MDataModule(GoldiproxDatamodule):
    ...
    def percentage_clean(self, selected_global_indices, set="train"):
        if set == "train":
            clean_indicator = self.indices_train.indicate_clean(
...
class ImageNetDataModule(GoldiproxDatamodule):
    def __init__(self, *args, **kwargs):
        ...
        if self.valset_fraction < 1:
            # all of this is only to make sure that the random subset is not very unbalanced
            def temp_valset(): return indices_ImageNet(
                self.data_dir,
                split="val",
                transform=self.transform
                if not self.valset_data_aug
...
class CINIC10RelevanceDataModule(GoldiproxDatamodule):
    ...
    def __init__(self, *args, **kwargs):
        ...
        valid_subset = list(np.nonzero(
            np.array(validation_dataset.targets) < 4.5)[0])
        ...
        self.indices_test_factory = lambda: Subset(
            indices_ImageFolder(self.data_dir + "/test",
                                transform=self.transform),
            test_subset,
        )
        if self.trainset_data_aug:
