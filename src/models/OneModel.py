...
class OneModel(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.hparams.percent_train < 1:
            ...
            selected_minibatch = torch.randperm(
                len(data))[:selected_batch_size]
        ...
        acc = accuracy(preds, target, task="multiclass",
    ...
    def validation_step(self, batch, batch_idx):
        _, data, target = batch
        logits = self.model(data) / self.temperature
        loss = self.loss(logits, target)
        ...
        acc = accuracy(preds, target, task="multiclass",
                       num_classes=self.datamodule.num_classes)
        self.log("val_loss_epoch", loss, on_epoch=True,
                 logger=True, sync_dist=True)
        self.log("val_acc_epoch", acc, on_epoch=True,
                 logger=True, sync_dist=True)
