...
from src.curricula.selection_methods import reducible_loss_selection, distance_gated_reducible_loss_selection, sublabels_reducible_loss_selection, threshold_reducible_loss_selection, reducible_loss_selection5, reducible_loss_selection6, reducible_loss_selection7, reducible_loss_selection_rand, reducible_loss_dropout_selection, reducible_loss_plus_conditional_entropy_selection, irreducible_loss_selection, gradnorm_ub_selection, ce_loss_selection, uniform_selection


def wmean(loss, irreducible_loss, zeta):
    if zeta is not None:
        weights = (loss.detach() - irreducible_loss.detach()) * zeta
        weights = torch.exp(weights)
        weights /= torch.sum(weights)  # Normalize weights to sum to 1

        # Compute the weighted mean of the regular losses
...
class MultiModels(pl.LightningModule):
    def __init__(
        ...
        self.zeta = avg_weights
        if track_all_selection:
            self.all_selection_methods = [
                reducible_loss_selection(),
                distance_gated_reducible_loss_selection(),
                sublabels_reducible_loss_selection(),
                threshold_reducible_loss_selection(),
                reducible_loss_selection5(),
                reducible_loss_selection6(),
                reducible_loss_selection7(),
                reducible_loss_selection_rand(),
                reducible_loss_dropout_selection(),
                reducible_loss_plus_conditional_entropy_selection(),
    ...
    def training_step(self, batch, batch_idx):
        ...
        selected_batch_size = max(
            1, int(batch_size * self.hparams.percent_train))
        ...
        selected_indices, metrics_to_log, irreducible_loss = self.selection_method(
            ...
            # not used by all methods, but needed for annealing
            current_epoch=self.current_epoch,
            num_classes=self.datamodule.num_classes,
            sub_label_mapping=self.datamodule.sub_label_mapping
        ...
        if isinstance(self.datamodule, Clothing1MDataModule) and self.hparams.track_all_selection:
            for name, selection_method in zip(self.all_selection_method_names, self.all_selection_methods):
                ...
                self.log(
                    ...
                    self.datamodule.percentage_clean(
                        global_index[selected_indices_method]),
                    on_step=True,
                    on_epoch=True,
                    logger=True,
                )
        ...
        if self.proxy_model is not None:
            ...
            self.manual_backward(wmean(proxy_model_loss, irreducible_loss, self.zeta))
            opt_proxy_model.step()

            # logging
            preds = torch.argmax(F.log_softmax(logits, dim=1), dim=1)
            proxy_model_acc = accuracy(
                preds, target, task="multiclass", num_classes=self.datamodule.num_classes)
            self.log(
                "proxy_train_loss",
        ...
        self.manual_backward(wmean(loss, irreducible_loss, self.zeta))
        opt_large_model.step()

        # training metrics
        preds = torch.argmax(F.log_softmax(logits, dim=1), dim=1)
        acc = accuracy(preds, target, task="multiclass", num_classes=self.datamodule.num_classes)
        ...
        if self.proxy_model is not None:
            ...
            self.log("std_proxy_loss", std, on_step=True,
                     on_epoch=True, logger=True)

            std = torch.std(loss)
            self.log("std_large_loss", std, on_step=True,
                     on_epoch=True, logger=True)
            ...
            if irreducible_loss is not None:
                ...
                spearman = self.spearman_correlation(
                    proxy_model_loss, irreducible_loss)
                self.log(
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.selection_method.bald:
            ...
            logits = torch.logsumexp(
                predictions, dim=1) - math.log(self.hparams.num_mc)
            loss = self.loss(logits, target)
        ...
        acc = accuracy(preds, target, task="multiclass", num_classes=logits.shape[1])
        self.log(
            "val_loss_epoch",
            loss.mean(),
            on_epoch=True,
    ...
    def configure_optimizers(self):
        ...
        if self.hparams.update_irreducible:
            ...
            opt_embedding_generator = hydra.utils.instantiate(
                config=unmask_config(self.optimizer_config),
                params=self.embedding_generator.parameters(),
                _convert_="partial",
            )
            for param_group in opt_irreducible_model.param_groups:
                param_group['lr'] = param_group['lr']/100
            return [opt_large_model, opt_irreducible_model, opt_embedding_generator]
        else:
