...
def compute_losses_with_sanity_checks(dataloader, model, device=None):
    ...
    if isinstance(dataloader.dataset, Clothing1M):
        ...
        idxs = torch.cat(idxs, dim=0).to(device)
        targets_temp = torch.cat(targets, dim=0).to(device)
        ...
        targets = torch.zeros(max_idx + 1, dtype=targets_temp.dtype, device=device)
...
def verify_correct_dataset_order(
    ...
    sorted_target = sorted_target.to(device)
    if isinstance(dataloader.dataset, Clothing1M):
